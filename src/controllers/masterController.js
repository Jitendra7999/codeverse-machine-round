...
import { adminMasterModel, adminUserModel, masterUserModel, publicUserDb } from '../utils/databases.js'

export const createUser = async (req, res) => {
    try {
        const masterId = req.headers.masterId
       const { name = '', email = '', password = '' } = req.body
        ...
        const model = await masterUserModel(masterId)

        const userExist = await model.findOne({ name  })
        ...
        const userCreated = await model.create({ name,email,password:hashPassword })

        res.status(201).send({ message: 'Master created', success: true, data: userCreated })
